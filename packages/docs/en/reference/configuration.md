...
For local runs (`pnpm dev` / Gatekeeper's `pnpm start`), the `dev` / `start` scripts of agent-core and gatekeeper (and gatekeeper's `db:migrate`) automatically load a `.env` file located in the package directory (e.g. `packages/agent-core/.env`) at startup (Node's native `--env-file-if-exists`). The frontend is excluded because Vite loads its own project-root `.env` natively. This flag requires Node v22.9.0+ (or v20.19.0+); this repository uses Node 24 in `.node-version`, `engines.node` and `docker/Dockerfile`, so the requirement is met. Copy the `.env.example` shipped with agent-core / gatekeeper to the corresponding `packages/<pkg>/.env` (the frontend has no `.env.example`). Environment variables already set (e.g. via a shell `export`) take precedence over `.env`.
