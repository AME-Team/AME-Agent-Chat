...
| メソッド | パス                 | 説明                                                        |
| -------- | -------------------- | ----------------------------------------------------------- |
| GET      | `/api/logs/download` | ログ全文を `text/plain` として返却（`.1` 世代があれば連結） |

::: warning 注意

- ログにはリクエスト詳細や SDK 呼び出し内容が含まれ得ます。この API はターミナル API と同じ **共有トークン（`x-terminal-token`）+ Origin 検証** を要求します。`LOG_API_ENABLED=false`（既定 `true`）でこの API 自体を無効化できます。
- レスポンスは末尾最大 **10MB** までに制限されます（超過時は `X-Log-Truncated: true` ヘッダ付きで末尾側を返します）。
  :::
