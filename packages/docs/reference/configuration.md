...
::: tip 環境変数の読み込み
ローカル実行（`pnpm dev` / Gatekeeper の `pnpm start`）では、各パッケージの `dev` / `start` スクリプト（Gatekeeper は `db:migrate` も）がパッケージ直下の `.env`（例: `packages/agent-core/.env`）を自動読み込みします（Node 標準の `--env-file-if-exists`）。本フラグは Node v22.9.0+（または v20.19.0+）が必要で、本リポジトリは `.node-version` / `engines.node` / `docker/Dockerfile` とも Node 24 のため要件を満たします。`.env.example` を `packages/<pkg>/.env` へコピーして利用してください。シェル等で既に設定済みの環境変数が `.env` の値より優先されます。
